...
def main(args):
    ...
    optimizer = optim.AdamW(model.parameters(), lr=5e-6, weight_decay=1e-9, betas=(0.9, 0.999))
    ...
    for epoch in range(args.epochs):
        ...
        for i, batch in enumerate(tqdm(train_loader, desc=f"Epoch {epoch + 1}/{args.epochs}")):
            ...
            #print('sfot_label', soft_label,soft_label.shape)
            pred = model(x)
            ...
            loss_sum += loss
        writer.add_scalar("Loss_epoch/train", loss_sum/len(dev_loader), epoch)
        print('trainloss-',loss_sum/len(dev_loader))
        loss_sum = 0
        scheduler.step()
        ...
        with torch.no_grad():
            ...
            print(epoch, 'val loss:', val_loss, 'val_eer:', val_eer)
